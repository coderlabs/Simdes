...
function resultData(data) {
    ...
    if (obj[6].length == 0) {
        ...
        $.each(obj[6], function (index, val) {
            $("#datalist").append(
                "<tr><td>" + val.judul.jenis + "</td>" +
                "<td>" + val.judul.judul + "</td>" +
                "<td>" + val.istilah + "</td>" +
...
function TombolBatal() {
    ...
    // remove class error
    $("#perdes_id").html("").removeClass('error');
    $("#istilah").val("").removeClass('error');
    // show
    $("#form-cari").show();
    $("#form-option").show();
    $("#tab-content").show();
