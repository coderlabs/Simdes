...
function TampilData(page) {
    OpenLoading();
    var term = $("#cari").val();
    var perdes_id = $("input[name=perdes_id]").val();
    $.ajax({
        type: "post",
        url: url + "/read?page=" + page,
        cache: false,
        data: 'perdes_id=' + perdes_id + '&term=' + term + token,
...
function resultData(data) {
    ...
    if (obj[6].length == 0) {
        ...
        $.each(obj[6], function (index, val) {
            var bab = (val.bab.length > 0) ? val.bab : "Tidak diset";
            var pasal = (val.pasal.length > 0) ? val.pasal : "Tidak diset";
            var judul = (val.judul.length > 0) ? val.judul : "Tidak diset";
            $("#datalist").append(
                "<tr><td>" + bab + "</td>" +
                "<td>" + pasal + "</td>" +
                "<td>" + judul + "</td>" +
                "<td><div class='btn-toolbar text-right'>" +
                "<a href='" + url_poin + '/' + val.id + "' title='Detail' class='btn btn-default fa fa-list'></a>" +
...
function TombolBatal() {
    ...
    $("label.error").hide();
    // remove class error
    $("#bab").val("").removeClass('error');
    $("#judul").val("").removeClass('error');
    $("#pasal").val("").removeClass('error');
...
//            pendahuluan: 'required'
        ...
        messages: {
//            bab: "Silahkan isi bab",
//            judul: "Silahkan isi judul",
//            pasal: "Silahkan isi pasal",
//            pendahuluan: "Silahkan isi pendahuluan"
        }
    })
});