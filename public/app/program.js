		...
		success: function(data) {
			...
			if (obj[6].length == 0) {
				...
				$.each(obj[6], function(index, val) {
					$("#datalist").append(
						"<tr><td>" + val.bidang + "</td><td>" + val.kode_program +"</td><td>" + val.program + "</td><td><div class='btn-group'><button  title='Edit' class='btn btn-sm btn-default fa fa-edit' onclick='EditData(" + val.id + ")'></button>" + "<button title='Hapus' class='btn btn-sm btn-danger fa fa-trash-o' onclick='HapusData(" + val.id + ")'></button></div></td></tr>"
					)
				});
			}
			$("#cari").focus();
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data, function(index, val) {
                if (val == val.id) {
                    $("#bidang").append("<option value='" + val.id + "' selected='selected'>" + val.bidang + "</option>")
                } else {
                    $("#bidang").append("<option value='" + val.id + "'>" + val.bidang + "</option>")
                }
            });
        },
        error: function(data) {}
