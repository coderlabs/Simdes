/**
 * Created by Edi Santoso on 5/19/2014.
    ...
    if (obj[6].length == 0) {
        ...
        $.each(obj[6], function (index, val) {
            ...
            $("#datalist").append(
                ...
                "<a title='Pelaksanaan Belanja' href='" + url + '/' + val.id + "' title='Edit' class='btn btn-sm btn-default fa fa-cogs' ></a>" + setRKA + setDPA +
                "</div>" +
                "</td></tr>"
            )
        });
...
function dataSuccess(data) {
    ...
    setTimeout(function () {
        ...
        $("#rkpdesa_id").val(data.rkpdesa_id);
...
function TombolBatal() {
    ...
    $("#kelompok_id").html("").removeClass('error');
    $("#volume_1").val("").removeClass('error');
    $("#satuan_1").val("").removeClass('error');
    $("#satuan_harga").val("").removeClass('error');
    $("#kegiatan").html("").removeClass('error');
    $("#jenis_id").html("").removeClass('error');
    $("#obyek_id").html("").removeClass('error');
    $("#rincian_obyek_id").html("").removeClass('error');
    // show
    $("#form-cari").show();
    $("#form-option").show();
    $("#tab-content").show();
...
function getKelompok() {
    // 1 : pendapatan 2:belnja 2:pembiayaan
    var akun_id = 2;
    $.ajax({
        type: "post",
...
// kegiatan untuk set data rkpdesa_id dan pagu_anggaran
$(function () {
    $("#kegiatan").change(function () {
        var data = $(this).val();
        var myArr = data.split('|');
        $("#rkpdesa_id").val(myArr[1]);
        $("#pagu_anggaran").val(myArr[2]);
        $("#kegiatan_id").val(myArr[0]);
    })
});
$(function () {
...
// ajax get data kegiatan dari RKPDesa
function getKegiatan() {
    $.ajax({
        type: "get",
        url: "ajax-list-kegiatan",
        success: function (data) {
            switch (data.Status) {
                case "Warning":
                    resultWarning(data);
...
function resulDataListKegiatan(data) {
    $("#kegiatan").html("").append("<option value=''>Pilih Kegiatan</option>");
    $.each(data, function (index, val) {
        $("#kegiatan").append("<option value='" + val.id + "|" + val.rpjmdesa_id + "|" + val.pagu_anggaran + "'>" + val.kegiatan + "</option>")
    });
}
...
$(function () {
    $("#myForm").validate({
        ...
        messages: {
            ...
            jumlah: "Jumlah dari perkalian satuan harga dan volume harus sama dengan pagu anggaran!"
        }
    })
});
// cek kelebihan anggaran belanja dari pagu_anggaran
$(function () {
    $("#satuan_harga").change(function () {
        var pagu_anggaran = $("#pagu_anggaran").val();
        var satuan_harga = $(this).val();
        var vol_1 = $("#volume_1").val();
        var vol_2 = $("#volume_2").val();
        var vol_3 = $("#volume_3").val();
        var volume_1 = (vol_1.length > 0) ? vol_1 : "1";
        var volume_2 = (vol_2.length > 0) ? vol_2 : "1";
        var volume_3 = (vol_3.length > 0) ? vol_3 : "1";
        var total = eval(satuan_harga) * eval(volume_1) * eval(volume_2) * eval(volume_3);
        $("#jumlah").val(total);
    })
});