...
function TampilData(page) {
	...
	$.ajax({
		type: "post",
		url: "master-provinsi/read?page=" + page,
		cache: false,
		...
		success: function(data) {
			...
			if (obj[6].length == 0) {
				...
				$.each(obj[6], function(index, val) {
					$("#datalist").append(
						"<tr><td>" + val.kode_prov + "</td><td>" + val.prov +"</td><td class='text-right'><div class='btn-toolbar'><button  title='Edit' class='btn btn-sm btn-default fa fa-edit' onclick='EditData(" + val.id + ")'></button><button title='Hapus' class='btn btn-sm btn-danger fa fa-trash-o' onclick='HapusData(" + val.id + ")'></a></div></td></tr>"
					)
				});
			}
			CloseLoading();
    ...
    $("#myForm").validate({
        ...
        rules: {
            ...
            messages: {
                ...
                provinsi: "Silahkan isi nama provinsi.",
