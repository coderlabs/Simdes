...
function TampilData(page) {
	...
	$.ajax({
		type: "post",
		url: "master-kecamatan/read?page=" + page,
		cache: false,
		data: 'term=' + term,
		success: function(data) {
			CekAuth(data)
			...
			if (obj[6].length == 0) {
				...
				$.each(obj[6], function(index, val) {
					$("#datalist").append(
						"<tr><td>" + val.prov + "</td><td>" + val.status +" "+ val.kab +"</td><td>" + val.kode_kec +"</td><td>" + val.kec +"</td><td class='text-right'><div class='btn-group'><button  title='Edit' class='btn btn-sm btn-default fa fa-edit' onclick='EditData(" + val.id + ")'></button><button title='Hapus' class='btn btn-sm btn-danger fa fa-trash-o' onclick='HapusData(" + val.id + ")'></a></div></td></tr>"
					)
				});
			}
			CloseLoading();
    ...
    $("#myForm").validate({
        ...
        rules: {
            ...
            messages: {
                ...
                kecamatan: "Silahkan isi kecamatan.",
