...
function TampilData(page) {
    OpenLoading();
    var term = $("#cari").val();
    var rpjmdesa_id = $("#rpjmdesa_id").val();
    $.ajax({
        type: "post",
        url: url + "/read?page=" + page,
        cache: false,
        data: 'rpjmdesa_id=' + rpjmdesa_id + '&term=' + term + token,
    ...
    if (obj[6].length == 0) {
        ...
        $.each(obj[6], function (index, val) {
            $("#datalist").append(
                ...
                "<a href='" + url_program + "/" + val.id + "'  title='Program' class='btn btn-sm btn-default'><i class='fa fa-laptop' ></i></a>" +
