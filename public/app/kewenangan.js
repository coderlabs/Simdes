		...
		success: function(data) {
			...
			if (obj[6].length == 0) {
				...
				$.each(obj[6], function(index, val) {
					$("#datalist").append(
						"<tr><td>" + val.kode_kewenangan + "</td><td>" + val.kewenangan +"</td><td><div class='btn-group'><button  title='Edit' class='btn btn-sm btn-default fa fa-edit' onclick='EditData(" + val.id + ")'></button>" + "<button title='Hapus' class='btn btn-sm btn-danger fa fa-trash-o' onclick='HapusData(" + val.id + ")'></button></div></td></tr>"
					)
				});
			}
			$("#cari").focus();
	...
	$.ajax({
		...
		success: function(data) {
			...
			setTimeout(function() {
				...
				$("#btnBatal").show();
				$("#id").val(val[0]);
				$("#kode_kewenangan").val(val[1]);
				$("#kewenangan").val(val[2]);
			}, 300);
			TombolTambah();
		},
		error: function(data) {
