    ...
    if (obj[6].length == 0) {
        ...
        $.each(obj[6], function (index, val) {
            $("#datalist").append(
                ...
                "<td>" + val.kewenangan.kewenangan + "</td>" +
                "<td>" + val.fungsi + "</td>" +
...
function dataSuccess(data) {
    ...
    setTimeout(function () {
        ...
        // inject data
        $("#id").val(data.id);
        $("#kewenangan_id").val(data.kewenangan_id);
...
function TombolBatal() {
    ...
    $("label.error").hide();
    // remove class error
    $("#kewenangan_id").html("").removeClass('error');
    $("#kewenangan").html("").removeClass('error');
    $("#kode_rekening").val("").removeClass('error');
    $("#fungsi").val("").removeClass('error');
    // show
    $("#form-cari").show();
    $("#form-option").show();
    $("#tab-content").show();
...
$(function () {
    $("#myForm").validate({
        ...
        messages: {
            kewenangan: "Silahkan isi pilih kewenangan",
            kode_rekening: "Silahkan isi kode rekening",
            fungsi: "Silahkan isi fungsi",
        }
    })
});
function getKewenangan() {
    $.ajax({
        type: "get",
        url: "ajax-list-kewenangan",
        success: function (data) {
            $.each(data, function (index, val) {
                $("#kewenangan").append("<option value='" + val.id + '|' + val.kode_rekening + "'>" + val.kewenangan + "</option>")
            });
        },
        error: function (data) {}
    });
...
$(function () {
    $("#kewenangan").change(function (event) {
        var data = $(this).val();
        var myArr = data.split('|');
        setTimeout(function () {
            $("#kewenangan_id").val(myArr[0]);
