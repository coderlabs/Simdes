...
function TombolBatal() {
    ...
    $("#bidang").html("").removeClass('error');
    $("#kode_rekening").val("").removeClass('error');
    $("#program").val("").removeClass('error');
    // show
    $("#form-cari").show();
    $("#form-option").show();
    $("#tab-content").show();
...
$(function () {
    $("#myForm").validate({
        ...
        messages: {
            fungsi: "Silahkan pilih Fungsi terlebih dahulu",
            bidang: "Silahkan pilih bidang",
            kode_rekening: "Silahkan isi kode rekening",
            program: "Silahkan isi program"
        }
    })
});
function getFungsi() {
...
function getBidang(fungsi_id) {
    $.ajax({
        ...
        success: function (data) {
            if (data == "") {
                $("#bidang").attr("disabled", "disabled").html("").append("<option value=''> Data kosong</option>");
            } else {
                $("#bidang").removeAttr('disabled').html("").append("<option value=''> Pilih Bidang</option>");
                $.each(data, function (index, val) {
                    $("#bidang").append("<option value='" + val.id + '|' + val.kode_rekening + "'>" + val.bidang + "</option>")
                });
            }
        },
        error: function (data) {}
